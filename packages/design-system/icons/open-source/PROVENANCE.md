...
| `open-down` | **Project-created.** Derived from the project `chevron-left` (90° rotation via SVG transform, `rotate(-90 64 64)`) | Monochrome, `fill="currentColor"`. Human decision 2026-07-28: reuse the existing project chevron rotated to point down, the same derivation `sort` used, rather than sourcing new artwork. Absent from v18 and from every tier, and human-retired in Sort round 2, so `NounSelect`'s dropdown arrow rendered blank in production. **Geometrically identical to `sort` by construction** — both are the project chevron rotated down. Kept as two separate nouns rather than an alias, because the arc's compatibility boundary forbids invented aliases and the two nouns have independent dispositions. |
...
| `check` | **Third-party, MIT.** Ant Design Icons, `packages/icons-svg/svg/outlined/check.svg`, from `ant-design/ant-design-icons` at immutable commit `6c18c63fbcfcf71dae09cd6bd6d63a48f8b688f1`. © 2018-present Ant UED; licence text vendored as `LICENSE-ant-design-icons`. | Normalized to `currentColor` (source declared no fill and defaulted to black); geometry otherwise verbatim, `viewBox="0 0 1024 1024"`. Editor-only root `class="icon"` and the XML prolog dropped. Human decision 2026-07-28: adopt Ant's outlined `check`, identified via `shadcn.io/icon/ant-design-check-outlined`; geometry taken from the pinned upstream commit rather than the aggregator, which publishes no path data. Reverses the Sort round 2 retirement of `check`: it had no artwork in any tier and rendered blank for `RemoveAccountSection`, `NotificationItem`, and `AddChannelForm`. As with `warning`, the aggregator credits contributor "HeskeyBaozi" while the upstream LICENSE names Ant UED as copyright holder, which is what this row records. |
