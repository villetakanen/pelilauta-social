...
Card presents a concise preview of one independently meaningful subject. It gives
...
content is a Surface or an owning application component rather than a Card.

### Architecture

Card is a server-renderable Svelte component. Its root is always an `article` and
remains passive when the Card has a destination. The required string title renders
...
| actions | at the bottom edge | Links and controls related to the Card's subject. |

The public schema preserves the v20 names and composition model:

| input | contract |
| :--- | :--- |
| `title` | Required string rendered as the Card heading. |
...
Card composes `.elevation-0` through `.elevation-4` from
`specs/design-system/surface/spec.md`; elevation 1 is the default. Card owns its
...
Card establishes inline-size containment. Its cover and responsive typography
therefore resolve against the Card's own width instead of the viewport. The title
uses Typography's h4 step and follows its container-driven downshift. Card has a
minimum block size of seven line units, giving its preview and action region stable
proportions in a listing.

### Constraints

The title is required and occupies at most two rendered lines. Card is used only
...
Body content inherits Card's low-emphasis foreground and is limited to a small
preview, normally one to three short paragraphs. Forms, multi-step workflows,
arbitrary documents, and repeating data sheets do not belong inside Card.

Card exposes no title-specific size, line-height, or tracking override. Typography
owns the h4 treatment and its narrow-container adaptation. A linked title has no
resting underline, restores the underline on hover, and receives a visible focus
...
anchored to the bottom of the Card. It is seven grid units high, uses standard gap
and horizontal padding, and bleeds through the Card's horizontal padding to its
...
Card does not supply application actions or business logic.

A cover is decorative, lazily loaded, cropped to 16:9, and may receive native
responsive image candidates and sizing hints. It bleeds through the Card's top and
horizontal padding while preserving the Card radius. Its lower edge carries an
...
area. The noun is decorative in Card because the required title already names the
subject; Card suppresses the Icon capability's default announcement in this
...
through text or another accessible state.

Card uses the large radius by default and permits `--cn-border-radius-card` to
override it. The radius applies to the Card, cover, image, and tint. A containing
layout owns Card width, arrangement, and spacing between Cards.

Card-owned title, eyebrow, description, and inherited body foregrounds meet WCAG
2.2 AA at every supported elevation in Light and Dark. Elevation 4 does not retain
a de-emphasised Card foreground that fails against its background. Supplied content
that sets another foreground owns that pairing.

Card renders its complete initial structure without browser globals, data fetching,
...
- Both applications can import and server-render Card from the local design
  system package.
...
- Browser checks verify linked-title focus, two-line truncation, cover sizing and
  tint, action-row geometry, container-responsive typography, and Card-owned
  foreground contrast.
...
- A destination never turns the article surface into an anchor or scripted
  whole-Card control.
- Card consumes the shared elevation utilities; it does not declare another
  background or shadow model.
- A linked decorative cover never creates a duplicate keyboard focus stop or
  accessible name.
- Actions remain at the bottom edge without Card acquiring margins or
  listing-layout rules.
- Card indicators add no accessible state of their own; consumers provide the
  state meaning separately.
- The initial server response contains the complete Card structure and content.

### Scenarios

```gherkin
Given a Card with a title and no optional content
When it renders
...
```gherkin
Given a Card with a destination
When it renders
...
```gherkin
Given a linked Card with a cover
When it renders
...
```gherkin
Given a linked Card whose eyebrow, body or actions contain another link or control
When a reader operates that nested element
Then only the nested element's action occurs
```

```gherkin
Given a Card with a noun and no cover
When it renders
...
```gherkin
Given a Card with a noun and a cover
When it renders
...
```gherkin
Given a Card with an eyebrow after a cover
When it renders
...
```gherkin
Given a Card whose actions contain two direct children
When it renders
...
```gherkin
Given a Card with notify and alert active
When it renders
Then the alert flag has visual precedence
```

```gherkin
Given a Card at elevation 4 in Light or Dark
When its title, eyebrow, description and inherited body foreground render
Then each Card-owned foreground meets WCAG 2.2 AA
```

```gherkin
Given a Card in a container narrower than the typography breakpoint
When it renders
Then its h4 title uses Typography's narrow-container step
```

```gherkin
Given Card rendered on the server without client-side JavaScript
