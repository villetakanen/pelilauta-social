...
Small is not on the scale: the mathematical step changes for legibility.
...
An element carries no margin; whatever holds it states the separation. The Content Area
capability governs the spacing and presentation of author-written markdown.
...
The block holding a thematic break states its separation, as every other block does.
...
separates it from the heading: as siblings in a content container, or inside a Content
Area region. Anywhere else it overlaps the heading.

Heading wrap and table typography remain undecided here. Link presentation and the
distinction between navigation and commands are specified in
...
```gherkin
Given a subtitle after a heading inside a Content Area region
When they render
