...
approver PR review. Use it only on explicit request. The mandatory separate
...
2. **Compatibility:** are behavior claims supported by cited source,
   deployed evidence, a governing spec, or a human decision? Are
...
4. **Verification reachability:** does every claimed check run through
...
7. **Generated and external sources:** can committed output drift from its
   ...
   preserved?
8. **Replaced surface:** when a change replaces a component or rule, were its
   selectors, tests, imperative APIs, dynamic inputs, persisted values, and
   accessibility semantics carried or knowingly dropped?
9. **Placement:** does each change sit in the file its spec assigns the concern
   ...
   already loses, or edited where it already wins, passes visual checks
   but places behavior in a file the spec excludes.
...
authorized for a later delta are not blockers. Non-blocking findings are fixed,
