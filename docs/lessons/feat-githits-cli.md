...
- **Integration identity.** PR #42 (`feat/githits-cli` → `main`); source head `48cfdea` passed the Netlify Node 22 deploy preview at `https://deploy-preview-42--pelilauta-social-dev.netlify.app`. The final documentation-only head remains gated on the same preview check before merge.
