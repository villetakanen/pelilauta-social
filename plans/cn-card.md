...
1. The design system owns a tested, server-renderable Svelte Card and its approved
   specification; its structure, states and visual language follow v20
2. A component book explains Card's public API and renders its variants from source
3. Every current `cn-card` consumer in Pelilauta uses the local Card, with existing
...
- **Front-page site and onboarding cards** — open, waits on the foundation; site
  ...
  invitation move to the local Card without changing their session or navigation logic
...
- **Workflow and status cards** — open, waits on the foundation; thread information,
  ...
  tooling adopt Card without changing the workflows hosted inside them
...
- **Card containment acceptance** — open, waits on all consumer migrations; real cards
...
- **Cyan Card retirement** — open, waits on all consumer migrations; card-only
