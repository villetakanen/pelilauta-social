...
Status: Slice 1 delivered 2026-07-30; slice 2 unstarted
Origin: written on `feat/ds-foundations`, an abandoned branch. Slice 1 reached
`main` by way of `feat/ds-navigation`, not by that branch. Salvaged here for the
2026-07-30 decisions below, which nothing else records — in particular the
reasoning behind deferring typography, which `plans/typography.md` builds on.
