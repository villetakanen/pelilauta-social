...
`DeleteClockApp.svelte`'s `.clock-preview` is a fourth instance: the clock beside
its label, the same icon-and-text shape, stated locally with the same gap.

## Remaining change

Spec and publish a listing row, then delete the local layouts in
`NotificationItem.svelte`, `ChannelInfoRow.astro`, `ChannelsList.astro`,
`ThreadListItem.svelte` and `DeleteClockApp.svelte` and compose the row. Until then
each local block stands as a marked stopgap and is not a pattern to copy.
