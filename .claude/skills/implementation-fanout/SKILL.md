...
This skill provides a Claude-specific execution tactic. It is not part of the portable delivery flow;
`.agents/skills/` stays vendor-neutral and never depends on this skill. Skipping
this tactic changes only speed, never what a slice must contain or prove.
...
A work unit is a subdivision of the implementation of that slice. Units never become
...
- Its brief is complete without surrounding plan context.
...
- The judgment whether a slice amends a governing spec, and the amendment
  itself.
...
1. Confirm the production outcome and plan for the slice exist. If not, plan first.
2. Draft the division. Verify each unit against the Division Contract and
   confirm the file sets are disjoint.
3. Resolve any in-scope ASK boundary with the operator before dispatching.
4. Dispatch the units concurrently as Sonnet subagents, one brief each.
5. Review every returned diff and check output directly. Treat a claimed passing
   check as unverified until you observe its real output.
6. Run the full deterministic checks of the slice over the combined result. Unit
   checks do not substitute for the slice gate.
7. Execute the serialized remainder, then hand integration to `delivery-slice`.

## Report

Report the division and why each unit was independent, work kept in the
orchestrator or serialized, checks run per unit and across the slice, any unit
result rejected or redone, and remaining work.
